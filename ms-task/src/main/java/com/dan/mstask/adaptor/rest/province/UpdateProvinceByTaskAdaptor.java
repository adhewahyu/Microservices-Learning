...
public class UpdateProvinceByTaskAdaptor extends RestAdaptor<UpdateProvinceByTaskRequest, ValidationResponse> {

    @Value("${config.integration.ms-masterdata.update-province-by-task}")
    private String updateProvinceByTaskUrl;
    ...
    @Override
    public ValidationResponse execute(UpdateProvinceByTaskRequest request) {
        this.setEnableDebug(true);
        this.setHttpMethod(HttpMethod.POST);
        this.setRestTemplate(restClientEnhance);
        this.setUrl(updateProvinceByTaskUrl);
        try{
    ...
    @Override
    protected EsbRequest generatePayload(UpdateProvinceByTaskRequest request) {
